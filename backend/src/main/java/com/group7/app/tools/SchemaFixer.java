...
public class SchemaFixer implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) {
    ...
    String infoSql =
        "SELECT column_name, data_type, character_maximum_length FROM information_schema.columns WHERE table_schema='public' AND table_name='questions' AND column_name IN ('title','author','content')";
    try (Connection c = dataSource.getConnection();
      ...
      try (var rs = s.executeQuery(infoSql)) {
        while (rs.next()) {
          log.info(
              "Before: column={} data_type={} char_max={}",
      ...
      String dropInsertPolicy =
          "DROP POLICY IF EXISTS allow_authenticated_insert_forum_media ON storage.objects";
      String createInsertPolicy =
          "CREATE POLICY allow_authenticated_insert_forum_media ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'forum-media' AND auth.role() = 'authenticated')";
      String dropSelectPolicy =
          "DROP POLICY IF EXISTS allow_authenticated_select_forum_media ON storage.objects";
      String createSelectPolicy =
          "CREATE POLICY allow_authenticated_select_forum_media ON storage.objects FOR SELECT USING (bucket_id = 'forum-media')";
      ...
      try (var rs = s.executeQuery(infoSql)) {
        while (rs.next()) {
          log.info(
              "After: column={} data_type={} char_max={}",
              rs.getString("column_name"),
              rs.getString("data_type"),
              rs.getObject("character_maximum_length"));
        }
