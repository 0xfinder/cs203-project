...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<?> handleValidationException(Exception ex) {
    var errors = (ex instanceof MethodArgumentNotValidException)
  ...
  public ResponseEntity<?> handleDataIntegrity(DataIntegrityViolationException ex) {
    String rootMsg = Optional.ofNullable(ex.getRootCause()).map(Object::toString).orElse(ex.getMessage());
    ...
    } else if (lc.contains("lessons_slug") || lc.contains("lessons_slug_key") || lc.contains("slug")) {
