  ...
  public UserMeResponse updateMe(
    ...
    if (displayName.length() < 2 || displayName.length() > 32) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "displayName must be 2 to 32 characters after trimming");
    }

    Role role = user.getRole();
    if (request.roleIntent() != null) {
      role =
          switch (request.roleIntent()) {
    ...
    if (avatarPath != null && !avatarPath.startsWith("uploads/" + userId + "/")) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "avatarPath must be in the authenticated user's uploads path");
    }
