...
class LessonSchemaV2E2ETest {
    ...
    void lessonSchemaV2SupportsAuthoringPlaybackAndGrading() throws Exception {
        ...
        MvcResult createLessonResult = mockMvc.perform(post("/api/lessons")
        ...
        long lessonId = objectMapper.readTree(createLessonResult.getResponse().getContentAsByteArray()).path("id").asLong();

        mockMvc.perform(post("/api/lessons/{lessonId}/steps", lessonId)
                        .with(auth(contributor))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsBytes(stepRequest(1, "TEACH")
                                .put("vocabItemId", rizz.getId()))))
                .andExpect(status().isCreated())
        ...
        mockMvc.perform(post("/api/lessons/{lessonId}/steps", lessonId)
                        .with(auth(contributor))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsBytes(mcqStepRequest(
                                2,
                                "What does \"rizz\" mean?",
        ...
        mockMvc.perform(post("/api/lessons/{lessonId}/steps", lessonId)
                        .with(auth(contributor))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsBytes(dialogueStepRequest(
                                3,
                                "A: I finished everything in five minutes.\nB: That sounds like cap."))))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.dialogueText").value("A: I finished everything in five minutes.\nB: That sounds like cap."));
        ...
        mockMvc.perform(post("/api/lessons/{lessonId}/steps", lessonId)
                ...
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.question.matchPairs[0].right").value("charisma or flirting ability"));
        ...
        mockMvc.perform(patch("/api/lessons/{lessonId}", lessonId)
                        .with(auth(contributor))
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        MvcResult playResult = mockMvc.perform(get("/api/lessons/{lessonId}/content", lessonId)
                        .with(auth(learner)))
                .andExpect(status().isOk())
        ...
        answers.addObject()
                .put("stepId", mcqStepId)
                .put("answer", "Charisma or flirting ability");
        ObjectNode matchAnswer = answers.addObject();
        matchAnswer.put("stepId", matchStepId);
        matchAnswer.set("answer", objectMapper.createObjectNode()
                .put("rizz", "charisma or flirting ability")
                .put("cap", "a lie"));
        answers.addObject()
                .put("stepId", shortAnswerStepId)
                .put("answer", "No cap");

        mockMvc.perform(post("/api/lesson-attempts")
                        .with(auth(learner))
                        .contentType(MediaType.APPLICATION_JSON)
                ...
                .andExpect(jsonPath("$.results[1].correctAnswer").value("rizz = charisma or flirting ability; cap = a lie"));

        mockMvc.perform(get("/api/user-lesson-progress")
                        .with(auth(learner)))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(get("/api/vocab-memory")
                        .with(auth(learner))
                        .param("due", "false")
                        .param("limit", "10"))
                .andExpect(status().isOk())
