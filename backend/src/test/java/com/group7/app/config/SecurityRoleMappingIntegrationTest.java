...
class SecurityRoleMappingIntegrationTest {
  ...
  private static final UUID CONTRIBUTOR_ID =
  ...
  void contributorWithGrantedAuthorityCanSubmitContent() throws Exception {
    ...
    org.assertj.core.api.Assertions.assertThat(contentCaptor.getValue().getTerm())
        .isEqualTo("rizz");
    org.assertj.core.api.Assertions.assertThat(contentCaptor.getValue().getDefinition())
        .isEqualTo("charisma");
  ...
  void contributorWithDatabaseRoleCanSubmitContent() throws Exception {
    when(jwtDecoder.decode("contributor-token"))
        .thenReturn(jwtFor(CONTRIBUTOR_ID, "contributor@example.com"));
  ...
  void moderatorWithDatabaseRoleCanAccessPendingContent() throws Exception {
    when(jwtDecoder.decode("moderator-token"))
        .thenReturn(jwtFor(MODERATOR_ID, "moderator@example.com"));

    mockMvc
        .perform(get("/api/contents/pending").header("Authorization", "Bearer moderator-token"))
        .andExpect(status().isOk())
