...
class ForumMappingServiceTest {
    ...
    void toQuestionResponseUsesUserProfileAndNestedAnswerMappings() {
        ...
        when(forumVoteService.getQuestionVoteSummary(1L, viewerId)).thenReturn(new VoteSummary(3, 1, "THUMBS_UP"));
        when(forumVoteService.getAnswerVoteSummary(2L, viewerId)).thenReturn(new VoteSummary(1, 0, null));
    ...
    void toQuestionResponseFallsBackToLegacyDisplayNameLookup() {
        ...
        when(userRepository.findFirstByDisplayNameIgnoreCase("OldName")).thenReturn(Optional.of(legacyUser));
        when(forumVoteService.getQuestionVoteSummary(5L, viewerId)).thenReturn(new VoteSummary(0, 0, null));
