...
const COLORS = [
  ...
  "bg-emerald-500", "bg-sky-500", "bg-rose-500",
];
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-5">
        ...
        {showAskForm && (
          <div className="bg-slate-900/80 border border-violet-500/30 rounded-2xl p-5 shadow-xl shadow-violet-950/50 space-y-3">
            ...
            {profile && !onboardingDone && (
              <p className="text-sm text-amber-400 bg-amber-900/20 border border-amber-500/30 rounded-lg px-3 py-2">
                Please <a href="/profile" className="underline">set a display name</a> before posting.
              </p>
            )}
            ...
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => { setShowAskForm(false); setNewTitle(""); setNewContent(""); }}
        ...
        {loading && (
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-slate-900/50 border border-white/5 rounded-2xl p-5 animate-pulse space-y-3">
              ...
              {isExpanded && (
                <div className="border-t border-white/5 bg-slate-950/40 px-5 py-4 space-y-4">
                  {(q.answers?.length ?? 0) === 0 ? (
                    <p className="text-sm text-slate-500 italic">This thread needs a lore drop immediately</p>
                  ) : (
                        ...
                        <li key={a.id} className="flex gap-3 group">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-xs font-semibold text-slate-300">{a.author}</span>
                              <span className="text-xs text-slate-600">{timeAgo(a.createdAt)}</span>
                            ...
                            <p className="text-sm text-slate-300 mt-0.5 leading-relaxed">{a.content}</p>
                          </div>
                        </li>
                      ))}
                    </ul>
                  ...
                  {canPost ? (
                    ...
                    <p className="text-xs text-amber-500/80 italic">
                      <a href="/profile" className="underline">Set a display name</a> to post answers.
                    </p>
                  ) : (
